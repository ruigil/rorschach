...
describe('store.namespace() persistence', () => {
  test('set writes to localStorage for a persisted key', () => {
    type S = { width: number }
    const ns = store.namespace<S>('persist-coding_file_write')
    ns.init({ width: 34 }, { persist: ['width'] })
    ns.set('width', 55)
    expect(localStorage.getItem('rorschach.store.persist-coding_file_write.width')).toBe('55')
  })

  test('init reads saved localStorage value for a persisted key', () => {
    localStorage.setItem('rorschach.store.persist-coding_file_read.width', '62')
    type S = { width: number }
    const ns = store.namespace<S>('persist-coding_file_read')
    ns.init({ width: 34 }, { persist: ['width'] })
  ...
  test('reset clears persisted key registration so set no longer writes', () => {
    ...
    // Re-create the namespace without persist and set — should not coding_file_write
  ...
  test('__resetStoreForTests clears persisted key registrations', () => {
    ...
    __resetStoreForTests()
    // After full reset, set on the same namespace should not coding_file_write to localStorage
