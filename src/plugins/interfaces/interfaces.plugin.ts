  ...
  lifecycle: onLifecycle({
    start: (_state, ctx) => {
      ...
      const httpRef = httpConfig
        ? ctx.spawn('http-0', createHttpActor(httpConfig), { server: null, connections: 0, activeSpans: {}, llmProviderRef: null, kgraphRef: null } as HttpState)
        : null
      const cliRef = cliConfig
        ? ctx.spawn('cli-0', createCliActor(), { ...CLI_INITIAL_STATE })
        : null
      const signalRef = signalConfig
        ? ctx.spawn('signal-0', createSignalActor(signalConfig), { seenIds: new Set<string>(), pending: new Map<string, string>(), activeSpans: {} })
        : null
  ...
  handler: onMessage<PluginMsg, PluginState>({
    config: (state, msg, ctx) => {
      ...
      const httpRef = newHttpConfig
        ? ctx.spawn(`http-${httpGen}`, createHttpActor(newHttpConfig), { server: null, connections: 0, activeSpans: {}, llmProviderRef: null, kgraphRef: null } as HttpState)
        : null
      const cliRef = newCliConfig
        ? ctx.spawn(`cli-${cliGen}`, createCliActor(), { ...CLI_INITIAL_STATE })
        : null
      const signalRef = newSignalConfig
        ? ctx.spawn(`signal-${signalGen}`, createSignalActor(newSignalConfig), { seenIds: new Set<string>(), pending: new Map<string, string>(), activeSpans: {} })
        : null
