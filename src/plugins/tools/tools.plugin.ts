...
const toolsPlugin: PluginDef<PluginMsg, PluginState, ToolsConfig> = {
  ...
  lifecycle: onLifecycle({
    start: (_state, ctx) => {
      ...
      if (slice?.visionActor) {
        visionRef = ctx.spawn('vision-actor-0', Vision({ model: slice.visionActor.model })) as unknown as ActorRef<ToolMsg>
      ...
      if (slice?.audioActor) {
        audioRef = ctx.spawn('audio-actor-0', Audio({ ttsModel: slice.audioActor.ttsModel, sttModel: slice.audioActor.sttModel, voice: slice.audioActor.voice ?? 'alloy' })) as unknown as ActorRef<ToolMsg>
      ...
      if (slice?.videoActor) {
        videoRef = ctx.spawn('video-actor-0', Video({ model: slice.videoActor.model, aspectRatio: slice.videoActor.aspectRatio, duration: slice.videoActor.duration, resolution: slice.videoActor.resolution, pollIntervalMs: slice.videoActor.pollIntervalMs, pollTimeoutMs: slice.videoActor.pollTimeoutMs })) as unknown as ActorRef<ToolMsg>
      ...
      return { state: {
        ...
        vision:     { config: slice?.visionActor ?? null, ref: visionRef, gen: 0 },
        audio:      { config: slice?.audioActor ?? null,  ref: audioRef,  gen: 0 },
        video:      { config: slice?.videoActor ?? null,  ref: videoRef,  gen: 0 },
  ...
  handler: onMessage<PluginMsg, PluginState>({
    config: (state, msg, ctx) => {
      ...
      let visionRef: ActorRef<ToolMsg> | null = null
      if (newVisionConfig) {
        const ref = ctx.spawn(`vision-actor-${state.vision.gen + 1}`, Vision({ model: newVisionConfig.model }))
      ...
      let audioRef: ActorRef<ToolMsg> | null = null
      if (newAudioConfig) {
        const ref = ctx.spawn(`audio-actor-${state.audio.gen + 1}`, Audio({ ttsModel: newAudioConfig.ttsModel, sttModel: newAudioConfig.sttModel, voice: newAudioConfig.voice ?? 'alloy' }))
      ...
      let videoRef: ActorRef<ToolMsg> | null = null
      if (newVideoConfig) {
        const ref = ctx.spawn(`video-actor-${state.video.gen + 1}`, Video({ model: newVideoConfig.model, aspectRatio: newVideoConfig.aspectRatio, duration: newVideoConfig.duration, resolution: newVideoConfig.resolution, pollIntervalMs: newVideoConfig.pollIntervalMs, pollTimeoutMs: newVideoConfig.pollTimeoutMs }))
