...
import { createCronActor, CRON_CREATE_TOOL_NAME, CRON_CREATE_SCHEMA, CRON_DELETE_TOOL_NAME, CRON_DELETE_SCHEMA, CRON_LIST_TOOL_NAME, CRON_LIST_SCHEMA } from './cron.ts'
import { createAudioActor, TRANSCRIBE_AUDIO_TOOL_NAME, TRANSCRIBE_AUDIO_SCHEMA, TEXT_TO_SPEECH_TOOL_NAME, TEXT_TO_SPEECH_SCHEMA } from './audio.ts'
import { createVideoActor, GENERATE_VIDEO_TOOL_NAME, GENERATE_VIDEO_SCHEMA } from './video-actor.ts'
...
import { createToolStatusActor, TOOL_STATUS_TOOL_NAME, TOOL_STATUS_SCHEMA } from './tool-status.ts'
...
const toolsPlugin: PluginDef<PluginMsg, PluginState, ToolsConfig> = {
  ...
  lifecycle: onLifecycle({
    start: (_state, ctx) => {
      ...
      if (webSearchConfig) {
        webSearchRef = ctx.spawn('web-search-0', createWebSearchActor(webSearchConfig)) as ActorRef<ToolMsg>
      ...
      const bashRef = ctx.spawn('bash-0', createBashActor(bashConfig ?? undefined)) as ActorRef<ToolMsg>
      ...
      const cronRef = ctx.spawn('cron-0', createCronActor()) as unknown as ActorRef<ToolMsg>
      ...
      const pdfRef = ctx.spawn('pdf-0', createPdfActor()) as ActorRef<ToolMsg>
      ctx.publishRetained(ToolRegistrationTopic, PDF_TOOL_NAME, { name: PDF_TOOL_NAME, schema: PDF_SCHEMA, ref: pdfRef })

      const fetchFileRef = ctx.spawn('fetch-file-0', createFetchFileActor()) as ActorRef<ToolMsg>
      ...
      const toolStatusRef = ctx.spawn('tool-status-0', createToolStatusActor()) as unknown as ActorRef<ToolMsg>
  ...
  handler: onMessage<PluginMsg, PluginState>({
    config: (state, msg, ctx) => {
      ...
      if (newWebSearchConfig) {
        webSearchRef = ctx.spawn(`web-search-${webSearchGen}`, createWebSearchActor(newWebSearchConfig)) as ActorRef<ToolMsg>
        ctx.publishRetained(ToolRegistrationTopic, WEB_SEARCH_TOOL_NAME, { name: WEB_SEARCH_TOOL_NAME, schema: WEB_SEARCH_SCHEMA, ref: webSearchRef })
      }

      const bashRef = ctx.spawn(`bash-${bashGen}`, createBashActor(newBashConfig ?? undefined)) as ActorRef<ToolMsg>
      ctx.publishRetained(ToolRegistrationTopic, BASH_TOOL_NAME,  { name: BASH_TOOL_NAME,  schema: BASH_SCHEMA,  ref: bashRef })
      ctx.publishRetained(ToolRegistrationTopic, WRITE_TOOL_NAME, { name: WRITE_TOOL_NAME, schema: WRITE_SCHEMA, ref: bashRef })
      ctx.publishRetained(ToolRegistrationTopic, READ_TOOL_NAME,  { name: READ_TOOL_NAME,  schema: READ_SCHEMA,  ref: bashRef })
      ...
      if (state.llmRef && newVisionConfig) {
        const ref = ctx.spawn(`vision-actor-${visionGen}`, createVisionActor({ llmRef: state.llmRef, model: newVisionConfig.model }))
      ...
      if (state.llmRef && newAudioConfig) {
        const ref = ctx.spawn(`audio-actor-${audioGen}`, createAudioActor({ llmRef: state.llmRef, ttsModel: newAudioConfig.ttsModel, sttModel: newAudioConfig.sttModel, voice: newAudioConfig.voice ?? 'alloy' }))
      ...
      const newPdfRef = ctx.spawn(`pdf-${pdfGen}`, createPdfActor()) as ActorRef<ToolMsg>
      ...
      const newFetchFileRef = ctx.spawn(`fetch-file-${fetchFileGen}`, createFetchFileActor()) as ActorRef<ToolMsg>
    ...
    _llmProviderUpdated: (state, msg, ctx) => {
      ...
      if (msg.ref && state.vision.config) {
        const ref = ctx.spawn(`vision-actor-${visionGen}`, createVisionActor({ llmRef: msg.ref, model: state.vision.config.model }))
        const visionRefTyped = ref as unknown as ActorRef<ToolMsg>
        ctx.publishRetained(ToolRegistrationTopic, ANALYZE_IMAGE_TOOL_NAME,  { name: ANALYZE_IMAGE_TOOL_NAME,  schema: ANALYZE_IMAGE_SCHEMA,  ref: visionRefTyped })
        ctx.publishRetained(ToolRegistrationTopic, GENERATE_IMAGE_TOOL_NAME, { name: GENERATE_IMAGE_TOOL_NAME, schema: GENERATE_IMAGE_SCHEMA, ref: visionRefTyped })
        visionRef = visionRefTyped
      ...
      if (msg.ref && state.audio.config) {
        const ref = ctx.spawn(`audio-actor-${audioGen}`, createAudioActor({ llmRef: msg.ref, ttsModel: state.audio.config.ttsModel, sttModel: state.audio.config.sttModel, voice: state.audio.config.voice ?? 'alloy' }))
        const audioRefTyped = ref as unknown as ActorRef<ToolMsg>
        ctx.publishRetained(ToolRegistrationTopic, TRANSCRIBE_AUDIO_TOOL_NAME, { name: TRANSCRIBE_AUDIO_TOOL_NAME, schema: TRANSCRIBE_AUDIO_SCHEMA, ref: audioRefTyped })
        ctx.publishRetained(ToolRegistrationTopic, TEXT_TO_SPEECH_TOOL_NAME,   { name: TEXT_TO_SPEECH_TOOL_NAME,   schema: TEXT_TO_SPEECH_SCHEMA,   ref: audioRefTyped })
        audioRef = audioRefTyped
