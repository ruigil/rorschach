...
document.querySelectorAll('.obs-subtab').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    const isMetrics = btn.dataset.subtab === 'metrics'
    metricsSummary.style.display = isMetrics && Object.keys(actorsMap).length > 0 ? 'flex' : 'none'
    obsLogControls.style.display = isMetrics ? 'none' : 'flex'
  })
})
