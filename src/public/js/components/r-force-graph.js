...
export class RForceGraph extends RorschachElement {
  constructor() {
    super()
    this.loadStyles(CSS)
    this._sim = null
  ...
  renderKnowledgeGraph(graph) {
    ...
    nodeGroup.append('circle')
      .attr('r', R)
      .attr('fill',         d => NODE_BG)
      .attr('stroke',       d => (LABEL_COLORS[d.labels[0]] || { stroke: DEFAULT_STROKE }).stroke)
  ...
  renderPlanGraph(graph, selectedId, onSelect) {
    ...
    const node = g.append('g').selectAll('g').data(nodes).enter().append('g')
      .attr('class', d => 'plan-node' + (d.id === currentSelected ? ' selected' : ''))
      .attr('cursor', 'pointer')
      .call(d3.drag()
        .on('start', (ev, d) => { if (!ev.active) sim.alphaTarget(0.3).restart(); d.fx = d.x; d.fy = d.y })
        .on('drag',  (ev, d) => { d.fx = ev.x; d.fy = ev.y })
